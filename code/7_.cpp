...
combinations get_type(CardsXBid& cxb){
	...
	int two_occ = 0;
	int js = 0;
	if (occurences.find('J') != occurences.end()) js = occurences['J'];

	for (auto& el : occurences){
		if (el.first == 'J') continue;
		if (el.second == 5) return combinations::FIVE_KIND;
		if (el.second == 4) four_occ += 1;
		if(el.second == 3) three_occ += 1;
		if (el.second == 2) two_occ += 1;

	}

	if (js == 5 || js == 4 || (js == 3 && two_occ==1) || (js == 2 && three_occ==1) || (js == 1 && four_occ==1)) return combinations::FIVE_KIND;
	if (four_occ==1 || js == 3 || (js == 2 && two_occ==1) || (js == 1 && three_occ == 1)) return combinations::FOUR_KIND;
	if ((three_occ==1 && two_occ == 1) || (two_occ==2 && js == 1)) return combinations::FULL_HOUSE;
	if (three_occ==1 || (two_occ==1 && js==1) || (js == 2)) return combinations::THREE_KIND;
	if (two_occ==2 || (two_occ==1 && js == 1)) return combinations::TWO_PAIR;
	if (two_occ == 1 || js == 1) return combinations::ONE_PAIR;
